          ...
          <div className={cn(PANEL_CLASS, "xl:col-span-8")}>
            <div className="mb-4 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-[3px] bg-warning" />Pending</span>
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-[3px] bg-primary" />Confirmed</span>
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weekLoadData}>
                    ...
                    />
                    <Bar dataKey="pending" stackId="status" name="Pending" fill="hsl(var(--warning))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="confirmed" stackId="status" name="Confirmed" fill="hsl(var(--primary))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="completed" stackId="status" name="Completed" fill="hsl(var(--success))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="cancelled" stackId="status" name="Cancelled" fill="hsl(var(--destructive))" radius={[0, 0, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <Badge className={`rounded-full border-0 px-2 text-[10px] ${statusStyles[booking.status]}`}>{booking.status}</Badge>
                    </div>

                    <div className="mt-2.5 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1.5"><User className="h-3 w-3" />{booking.userName}</span>
                      {booking.userPhone && (
                        <a href={`tel:${booking.userPhone}`} className="flex items-center gap-1.5 transition-colors hover:text-foreground">
                          <Phone className="h-3 w-3" />{booking.userPhone}
                        </a>
                      )}
                      <span className="flex items-center gap-1.5"><Calendar className="h-3 w-3" />{formatDate(booking.date)}</span>
                      <span className="flex items-center gap-1.5"><Clock className="h-3 w-3" />{booking.startTime} - {booking.endTime}</span>
                    </div>
