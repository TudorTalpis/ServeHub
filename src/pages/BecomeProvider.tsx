...
const BecomeProvider = () => {
  ...
  const slugTaken =
    slug.trim().length > 0 && state.providerProfiles.some((p) => p.slug.toLowerCase() === slug.trim().toLowerCase());
    ...
    return (
      <div className="container mx-auto max-w-md px-4 py-16 text-center animate-fade-in">
        <h1 className="text-2xl font-bold mb-2">
          Application{" "}
          {blockingApplication.status === "PENDING" ? "Submitted" : blockingApplication.status.toLowerCase()}
        </h1>
        <p className="text-muted-foreground text-sm">
          Your application for "{blockingApplication.name}" is currently <strong>{blockingApplication.status}</strong>.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-8 animate-fade-in">
      ...
      <p className="text-sm text-muted-foreground mb-8">
        Fill out your profile information and add photos to get started.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-card space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g. Alina's Nail Studio"
            />
          </div>
          <div>
            <label htmlFor="custom-slug" className="text-sm font-medium flex items-center gap-1.5">
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-card space-y-4">
            ...
            <div className="mt-2 flex items-center gap-4">
              {avatar ? (
                ...
                <label
                  ...
                  className="flex h-20 w-20 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-muted-foreground/30 hover:border-primary/50 transition-colors"
                >
          ...
          <div>
            <label htmlFor="gallery-photos" className="text-sm font-medium">
              Gallery Photos * (at least 1, up to 6)
            </label>
            <div className="mt-2 grid grid-cols-3 gap-2" id="gallery-photos">
              ...
              {galleryPhotos.length < 6 && (
                <label
                  ...
                  className="flex aspect-[4/3] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-muted-foreground/30 hover:border-primary/50 transition-colors"
                >
                  <Upload className="h-5 w-5 text-muted-foreground/50" />
